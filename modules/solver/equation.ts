 ...
 * 一元方程解析 / 编译器（SOLVE 功能）。
 *
 * 参考 feat/solve-newton 分支实现移植：完全手写 tokenizer + 递归下降解析器，
 * 输出 AST 后由闭包求值，标识符 / 函数 / 常数全走白名单，绝不 eval。
 *
 * 支持的语法：
 ...
 *   不带等号时视为 f(x) = 输入表达式
 * - 变量：单字母标识符（a-zA-Z，e 保留为自然常数），编译时指定求解变量，
 *   其余自由变量会报错
 * - 三角函数按角度单位（DEG / RAD / GRA）求值
 */
...
function tokenize(src: string): Token[] {
    ...
    while (i < src.length) {
        ...
        throw new EquationError(`非法字符 "${ch}"（位置 ${i + 1}）`);
    }
...
/** 解析并收集方程中出现过的全部变量名（不编译） */
export function collectVariables(src: string): Set<string> {
    const text = src.trim();
    if (text === "") {
        throw new EquationError("方程为空");
    }
    const ast = new Parser(tokenize(text)).parseEquation();
    return collectVars(ast);
}
